...
class Tracker(object):
    ...
    def __init__(self, model_path: str) -> None:
        ...
        # 加载模型创建_Module
        self.net = MNN.nn.load_module_from_file(model_path, ['z', 'x'], \
                                           ['pred_boxes', 'score_map', 'size_map', 'offset_map'], 
                                            precision_mode = 1,
                                            runtime_manager=self.rt)
        

        self.template_factor = 2.0
        self.search_factor = 5.0 #4.0
        self.template_size = 192 #128
        self.search_size = 384 #256
        self.stride = 16
    ...
    def initialize(self, image, target_bb):
        ...
        self.z = expr.convert(self.z, expr.NCHW)
    ...
    def track(self, image):
        ...
        # run
        output_var = self.net.forward([self.z, x])


        pred_boxes_arr = output_var[0].read()
        score_map_arr = output_var[1].read()
        size_map_arr = output_var[2].read()
        offset_map_arr = output_var[3].read()
...
def run(tracker, video_path):
    ...
    while cap.isOpened():
        ...
        if frame_count == 1:
            ...
            bbox = (744, 417, 42, 95)
            frame_track = cv2.cvtColor(frame_track, cv2.COLOR_BGR2RGB)
...
if __name__ == "__main__":
    ostrack = Tracker(model_path="model/ostrack-384-ep300.mnn")
